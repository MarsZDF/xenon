...
|-------|--------------------------------|-----------|
| **Truncated XML** (cut-off response) | ❌ `XMLSyntaxError: unexpected end of data` | ✅ Auto-closes open tags & structures |
| **Conversational Fluff** ("Here is...") | ❌ `XMLSyntaxError: Start tag expected` | ✅ Extracts XML from text |
| **Malformed Attributes** (`id=1`) | ❌ `XMLSyntaxError` | ✅ Adds missing quotes |
| **Memory Usage** | ⚠️ Loads full DOM | ⚡ Streaming mode (constant memory) |
| **Security** | ⚠️ Vulnerable to XXE/Billion Laughs by default | 🔒 Secure-by-default (`TrustLevel`) |
